...
- **`WordPress.Security.EscapeOutput.ExceptionNotEscaped` in `Infrastructure/WpPostRepository.php` (×2)** — flags `WP_Error::get_error_message()` being passed into an internal exception constructor as if it were unescaped HTML output. Why this is safe to suppress here, spelled out explicitly (not just in the inline comment) since this is a public OSS plugin and the reasoning should survive a reviewer who never opens the source file:
  - The value only ever becomes a `code`/`message` field inside a JSON error body returned from `RestResponseFactory::error()` — it is a **REST JSON response value**, never HTML markup.
  - It is **never echoed directly into an HTML page** anywhere in this plugin; there is no template or admin-notice rendering path that touches this string.
  - The JSON encoding itself is performed by **WordPress's own REST framework** (`WP_REST_Response`/`WP_Error` → `wp_json_encode()` in WordPress core's REST server), which is the correct, already-safe serialization boundary for this value — applying `esc_html()` or similar HTML-escaping on top would be redundant at best and would corrupt the string (double-encoding entities) at worst.
  - The sniff remains **active project-wide** for every other file — this is a targeted, justified exception for two specific lines, not a weakening of the security ruleset.
- **`WordPress.WP.AlternativeFunctions.parse_url_parse_url` in `Domain/DraftPayload.php`** — recommends `wp_parse_url()` over `parse_url()` for cross-PHP-version consistency. `parse_url()` is used here **intentionally**, specifically to preserve `Domain/`'s WordPress-free status (see [Layering](#layering)) — `wp_parse_url()` is a WordPress core function, and calling it from `Domain/` would reintroduce exactly the dependency this design revision's core fix (item 4 in [Design decisions confirmed in review](#design-decisions-confirmed-in-review)) removed. The trade-off (a theoretical cross-PHP-version URL-parsing edge case vs. keeping the value object dependency-free and testable with zero WordPress bootstrap) was judged to favor the latter for this narrow, well-formed-URL validation use.
