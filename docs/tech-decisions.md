...
**Decision:** `url` is no longer a required field on `POST /draft` or `material_capture.createDraft` — an empty/absent `url` is accepted, skips format validation entirely (a non-empty one still must be a plausible `http`/`https` URL), and the rendered post body simply omits the "元URL: " line rather than showing it dangling with nothing after it.

**Context:** Added to support sharing a Chrome text *selection* (as opposed to a whole page) with the intent of populating the Confirm screen's メモ field from the selected text — see [docs/phase3-android-app-design.md](phase3-android-app-design.md)'s IntentParser revision 2. Confirmed on real devices that Chrome's text-selection share does not reliably include the source page's URL at all (site-dependent — present sometimes on some sites, absent on others). Requiring `url` client-side would have meant the user *must* manually type in a URL just to save a plain memo captured from selected text, which defeats the point of a quick-capture flow.

**Alternatives considered:**
- **Relax only the Android client's Save-button gating, keep `url` server-required.** Rejected — pressing Save would just move the same failure to a `400 missing_required_field` after a network round-trip, worse UX than a disabled button, not better.
- **Require the client to fabricate a placeholder URL when none is known (e.g. the app's own scheme, or the site's homepage).** Rejected — a fabricated URL is actively misleading in the saved draft (looks like real provenance data when it isn't) and would need to be distinguished from real URLs everywhere downstream (the GitHub Actions pipeline, Codex, the human reviewing drafts).

**Why this shape:** `Domain\DraftPayload::create()` is the single place this invariant is enforced (unchanged architecture — see [docs/phase2-wordpress-plugin-design.md](phase2-wordpress-plugin-design.md#layering)), so both REST and XML-RPC get the relaxed rule for free with no duplicated logic. Format validation for a *non-empty* `url` is untouched — this is purely about whether the field may be absent, not a loosening of what a supplied URL must look like (still `http`/`https` only, still rejects `javascript:`/`ftp:` schemes).
