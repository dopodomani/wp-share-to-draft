...
- Android CI (.github/workflows/android-ci.yml): on push to `main`/PRs touching `android/**`, runs `:core:test`, `:core:ktlintCheck`, `:app:testDebugUnitTest`, `:app:lintDebug`, `:app:assembleDebug` on Ubuntu with a real Android SDK — closes the verification gap the secondary PC has for `:app`. No secrets required (network tests use MockWebServer only).
